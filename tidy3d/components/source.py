...
class GaussianBeam(AngledFieldSource, PlanarSource, BroadbandSource):
    ...
    waist_distance: float = pydantic.Field(
        ...
        "is behind the source plane. When ``direction`` is ``+`` and ``waist_distance`` "
        "is negative, the waist is in front of the source plane.",
        units=MICROMETER,
    )
...
class AstigmaticGaussianBeam(AngledFieldSource, PlanarSource, BroadbandSource):
    ...
    waist_distances: Tuple[float, float] = pydantic.Field(
        ...
        "is behind the source plane. When ``direction`` is ``+`` and ``waist_distances`` "
        "are negative, the waist is in front of the source plane.",
        units=MICROMETER,
    )
